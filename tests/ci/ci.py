...
def _configure_jobs(
    ...
    if "#no_ci_cache" in commit_tokens:
        done_files = []
    else:
...
def main() -> int:
    ...
    ### RUN action: start
        ...
        if is_build_job(check_name):
            # check build report is present
            build_result = BuildResult.load(check_name)
            if build_result:
                print(
                    f"Build report already present - job will be skipped with status: [{build_result.status}]"
                )
                print("::group::Build Report")
                print(build_result.as_json(), indent=2)
                print("::endgroup::")
